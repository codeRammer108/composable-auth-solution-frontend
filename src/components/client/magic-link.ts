  ...
  const signInLinkRequested = (async () => {
    try {
      ...
      if (err.name === "UserNotFoundException") {
        //user doesn't exist in the system, sign them up and call for magic link again
...
async function signUpNewUser(username:string){
  const payload = {
    "email": username
  };
  try {
    let url = "https://5giqb1mash.execute-api.us-west-2.amazonaws.com/dev/add-user";
    ...
    if (response.ok) {
      ...
      //request for magic link once the user is registered
